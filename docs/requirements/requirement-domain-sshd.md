...
1b. **menu numbers** are owned by `requirement-shell-cli-default-interaction` (front **1** client-side, **2** server-side, **8** self-management, **9** Exit; client **11** dns, **12** ssh, **13** download, **14** upload reserved; server **21** status, **22–24** start/stop/restart; submenu **0** Back). This file owns the **handlers**. Choosing **11** (or typing `dns`) runs `sshd_cmd_dns`. Choosing **12** (or typing `ssh`) runs `sshd_cmd_ssh` (numbered Host pick, then **user** with default). Choosing **13** (or typing `download`) runs `sshd_cmd_download`. **14** upload stays reserved until that handler ships. `port` / `config` / `host-keys` / `auth-keys` stay typed. The Host pick **MUST NOT** reuse front Exit **9** (leave Host pick with **0**).  
