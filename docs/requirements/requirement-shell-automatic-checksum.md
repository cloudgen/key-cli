...
This requirement is the **project Single Source of Truth** for **automatic companion-digest integrity** of the key-cli POSIX shell tool: downloading a SHA-256 sidecar next to the install channel, verifying install/self-update downloads, and reporting the process **transparently** (companion **link**, expected **value**, and verification **result**).
...
**Must not confuse with:** Embedding a hash of `./key-cli` *inside* `./key-cli`; requiring operators to set `CHECKSUM` for every install; claiming independent host authenticity from same-channel SHA-256 alone.
...
| Box | Meaning | Example |
|-----|---------|---------|
| You / this login | Someone installing or updating from the channel | `curl … \| sh` · `key-cli self-update` |
...
| Publisher ships `key-cli.sha256` beside the program | Embedding the hash *inside* the program file |

| Surface | What you open | What for |
|---------|---------------|----------|
| `./key-cli.sha256` | Companion digest | Publisher sidecar |
| Install / self-update output | Messages | Link, value, result |

| You do… | What it means | What you type |
|---------|---------------|---------------|
| Install from the channel | The tool downloads the program **and** the sidecar, then reports whether the bytes matched. A mismatch **must fail**. | `curl -fsSL …/key-cli \| /bin/sh` |
| Read help | Help and about **must not** tell people to set `CHECKSUM`. | `key-cli help` |
...
| **In-repo publisher SSOT** | Companion file **`key-cli.sha256`** (bare SHA-256 hex of `./key-cli`) **MUST** ship next to the installable script for the release channel. |
| **Regenerate on change** | After every edit to `./key-cli` that is published, regenerate `key-cli.sha256` so automatic mode can match. |
...
| **Outside payload** | Pin is env/operator/CI for that process — **MUST NOT** be embedded inside `./key-cli` as a self-hash of that file. |
...
1. Hash of `./key-cli` stored **inside** `./key-cli` as the verify target.  
...
1. Product root **`README.md` MUST** explain **automatic** checksum as the default: algorithm (SHA-256), companion URL pattern, in-repo `key-cli.sha256`, match / mismatch / missing outcomes.  
...
| **Channel SSOT** | `SCRIPT_URL` default `https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/main/${APP_NAME}` → `https://raw.githubusercontent.com/cloudgen/key-cli/main/key-cli` |
| **Companion URL** | `${SCRIPT_URL}.sha256` → `https://raw.githubusercontent.com/cloudgen/key-cli/main/key-cli.sha256` |
| **In-repo companion** | `key-cli.sha256` (bare 64-char hex) |
...
2. Embed the expected SHA-256 of `./key-cli` **inside** `./key-cli` as verification.  
...
5. Document same-origin `CHECKSUM=$(curl …/key-cli.sha256)` as higher assurance than automatic companion verification.  
...
Integrity work for key-cli is **not done** if any of the following fail:
...
| `./key-cli` | Ship unit implementation |
| `key-cli.sha256` | In-repo companion digest |
