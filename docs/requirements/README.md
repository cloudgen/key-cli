...
**Current state (2026-09-16 — key-cli 2.0.2):** key-cli backups and restores this login’s whole `~/.ssh` folder as dated `tar.gz` under `/var/key-cli`. Type 1 is passwordless `sudo key-cli backup` / `restore`. Type 2 is **key-adm**. Domain SSOT is **`requirement-domain-key`** (`requirement-domain-sshd` is Superseded). TTY tree is **1 keys / 2 on-behalf / 8 self-management / 9 Exit**. Termux / Git Bash / Windows cmd: Type 1/2 unused; no OpenSSH `pkg`; no wake-lock. Proof: `reviews/test-plan.md` · `reviews/requirement-test-matrix.md` · `tests/run.sh`. Registry: `index.md`. Specialized from bootstrap origin **selfmanaged** (A → B). Product version SSOT: `VERSION="2.0.2"`. Do **not** invent additional requirement paths without a real ownership gap — verify on disk and register new files in `index.md` in the same change.
